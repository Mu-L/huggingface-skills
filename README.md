...
| `huggingface-local` | Run Hugging Face models locally with llama.cpp and GGUF on CPU, Metal, CUDA, or ROCm. Covers Hub discovery with llama.cpp local-app pages, quant selection, exact GGUF file lookup, conversion, and OpenAI-compatible local serving. | [SKILL.md](skills/huggingface-local/SKILL.md) |
